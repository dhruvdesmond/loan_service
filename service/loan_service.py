...
class LoanService:
    ...
    @staticmethod
    def add_repayment_by_customer(loan_uuid: str, repayment_uuid: str, AmountData: AmountInsertModel) -> GenericResponseModel:
        """
        ...
        for repayment in loan_data.repayments:
            ...
            if str(repayment.uuid) == repayment_uuid:
        ...
        if repayment_data.amount > AmountData.amount:
            logger.error(extra=context_log_meta.get(), msg="Amount smaller than required")
            return GenericResponseModel(status_code=http.HTTPStatus.BAD_REQUEST,
                                        message="Amount smaller than required")

        minAmount = repayment_data.amount
        maxAmount = Repayment.get_remaining_amount(repayment_data.loan_id)

        if minAmount > AmountData.amount:
            logger.error(extra=context_log_meta.get(), msg="Amount smaller than required")
            return GenericResponseModel(status_code=http.HTTPStatus.BAD_REQUEST,
                                        message="Amount smaller than required")
        if AmountData.amount > maxAmount:
            logger.error(extra=context_log_meta.get(), msg="Amount greater than required")
            return GenericResponseModel(status_code=http.HTTPStatus.BAD_REQUEST,
                                        message="Amount greater than required")

        Repayment.update_repayment_status(repayment_data.loan_id,AmountData.amount)
